function modal() {
    ...
    function desativarModal(modal, ultimoBotaoAtivo) {
        ...
        const limparFormularioModal = modal.querySelector('form');
        botaoFecharModar.forEach(botao => {
            botao.addEventListener('click', () => {
                modal.style.display = 'none';
                document.body.style.overflow = 'auto';
                ultimoBotaoAtivo.focus();
                if(limparFormularioModal) {
                    limparFormuLarioModal(modal, limparFormularioModal);
                }
            });
        });
    }

    function limparFormuLarioModal(modal) {
        const mensagemDeErro = formulario.querySelectorAll('[data-js="mensagem-de-erro"]');
        limparFormulario.reset();

        mensagemDeErro.forEach(mensagem => {
