...
function formulario() {
    ...
    function enviarFormulario(formulario) {
        const botaoDeEnvio = formulario.getElementsByTagName('button');
        console.log(botaoDeEnvio);
        formulario.addEventListener('submit', (evento) => {
            evento.preventDefault();
            if (formulario.checkValidity()) {
                pegarDadosDoFormulario(formulario);
                despachaMensagemDeEnvio(formulario);
                botaoDeEnvio[0].setAttribute('data-js', 'fechar-modal');
                botaoDeEnvio[0].setAttribute('data-abrir-modal', 'mensagem-formulario-enviado');
                formulario.reset();
            }
        });
    }
